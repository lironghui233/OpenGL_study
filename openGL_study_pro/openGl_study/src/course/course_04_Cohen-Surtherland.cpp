/*
	Describe: Line Clipping Algorithm (Cohen-Surtherland)
	Author: Leoh
	Data: 2024/9/5
*/
...
int cohensurtherland_lineclip(Rectangle rect, int& x0, int& y0, int& x1, int& y1)
{
	int accept, done;
	float x, y;
	accept = 0;
...
void Keyboard(unsigned char key, int x, int y)
{
	...
	{
	case 'c':
		cohensurtherland_lineclip(rect, x0, y0, x1, y1);
		glutPostRedisplay();
		break;
	case 'r':
		Init();
